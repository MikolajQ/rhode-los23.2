...
- `bionic` — hartowanie GrapheneOS (strony ochronne stosu wątków, read-only `__stack_chk_guard`, `explicit_bzero`,
  blokujący `getrandom`). **Bez** zamiany alokatora scudo → jemalloc (patrz niżej).
...
- **jemalloc zamiast scudo** (`bionic`, `build`, `soong`, `debuggerd`): scudo to alokator utwardzony przeciw
  exploitom pamięci; zamiana dla kilku procent wydajności osłabia bezpieczeństwo — sprzeczne z resztą zestawu.
...
- **~150 mikrooptymalizacji rmp22** w `frameworks/base` („Optimizing…”, „Throttling…”, „Prevent system_server from
  restarting due to app issues”): nieprzejrzane przez LineageOS/AOSP, dotykają gorących ścieżek WindowManagera
  i SystemUI, wiele bez pomiarów; to one najczęściej konfliktują przy aktualizacji. Na SD680 zysk nieweryfikowalny.
- **Wyciszanie logów** (~60 commitów): kosmetyka, zero korzyści dla użytkownika, dużo konfliktów.
- **Flagi kompilacji** `-O3`, ThinLTO, unified LTO, `-march` dla Kryo: wydłużają build i zwiększają ryzyko
  miscompile; zysk w pomiarach typowo < 2%.
