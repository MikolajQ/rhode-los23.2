...
dhas product.img /priv-app/GmsSupervision/base.apk && ok "GmsSupervision: base.apk" || bad "GmsSupervision bez base.apk"
dls product.img /priv-app/GmsSupervision | grep -q '^split_' && ok "GmsSupervision: split APK obecny" || warn "GmsSupervision bez splitu (OK, jeśli użyto nodpi)"
